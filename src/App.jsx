...
import {
  ...
  Clipboard, UserPlus, Pencil, History,
} from 'lucide-react'
import { cashDifferenceLabel, cashExpected, cashRemoved, lowStock, money, paymentMethodTotals, paymentTotal, paymentsMatchTotal, purchaseTotal, saleTotal } from './lib/format'
...
function App({ workspace }) {
  ...
  const loadData = async () => {
    ...
    const [productsResult, inventoryResult, salesResult, salePaymentsResult, customersResult, suppliersResult, purchasesResult, sessionsResult, movementsResult, reconciliationsResult, financeResult, teamResult, invitationsResult] = await Promise.all([
      supabase.rpc('list_store_products_v2', { p_store_id: workspace.store.id }),
      supabase.from('inventory_movements').select('*').eq('store_id', workspace.store.id).order('created_at', { ascending: false }).limit(100),
    ...
    if ([productsResult, inventoryResult, salesResult, salePaymentsResult, customersResult, suppliersResult, purchasesResult, sessionsResult, movementsResult, reconciliationsResult, financeResult, teamResult, invitationsResult].some((result) => result.error)) notify('Não foi possível atualizar todos os dados permitidos para seu acesso.')
    ...
    setInventoryMovements((inventoryResult.data || []).map(mapInventoryMovement))
  ...
  const updateProduct = async (productId, values) => {
    ...
    if (error) {
      notify(error.message.includes('products_store_id_sku_key') ? 'Já existe um produto com este código.' : 'Não foi possível atualizar o produto.')
      return false
    }
    await loadData(); notify(values.active ? 'Produto atualizado.' : 'Produto inativado sem apagar o histórico.'); return true
  }
    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <div className="content">
          ...
          {page === 'Caixa' && <Cashier workspace={workspace} activeCash={activeCash} sessions={cashSessions} movements={cashMovements} reconciliations={cashReconciliations} sales={sales} openCash={openCash} moveCash={moveCash} closeCash={closeCash} reconcileCash={reconcileCash} />}
          {page === 'Financeiro' && <Finance entries={financialEntries} customers={customers} suppliers={suppliers} createEntry={createFinancialEntry} settleEntry={settleFinancialEntry} />}
...
function Dashboard({ store, products, sales, activeCash, goTo }) {
  ...
  return <>
    ...
    <section className="metrics">
      ...
      <Metric label="Itens em estoque" value={products.filter((product) => product.active).reduce((s,p) => s + p.stock, 0)} note={`${alerts.length} pedem reposição`} warning={alerts.length > 0} />
...
function POS({ products, customers, activeCash, cart, add, setQty, finish, processing }) {
  ...
  const shown = products.filter(p => [p.name, p.sku, p.category, p.size].some((value) => value?.toLowerCase().includes(query.toLowerCase())))
  const total = saleTotal(cart)
  useEffect(() => {
...
function Products({ products, suppliers, add, goTo, createProduct, updateProduct, createSupplier, canManage }) {
  ...
  const [query, setQuery] = useState('')
  const shown = products.filter((product) => [product.name, product.sku, product.category, product.size, product.supplierName].some((value) => value?.toLowerCase().includes(query.toLowerCase())))
  return <>
    {editing !== null && canManage && <ProductForm product={editing || undefined} products={products} suppliers={suppliers} onCancel={() => setEditing(null)} onSave={async (values) => { const saved = editing ? await updateProduct(editing.id, values) : await createProduct(values); if (saved) setEditing(null) }} />}
    {supplierOpen && canManage && <SupplierForm onCancel={() => setSupplierOpen(false)} onSave={async (values) => { if (await createSupplier(values)) setSupplierOpen(false) }} />}
    <section className="panel"><div className="panel-head"><div><span className="section-label">CATÁLOGO REAL</span><h3>{products.filter((product) => product.active).length} produtos ativos</h3></div><div className="panel-actions">{canManage && <button className="secondary" onClick={() => setSupplierOpen(true)}><Truck size={17}/>Novo fornecedor</button>}{canManage && <button className="secondary" disabled={!suppliers.length} onClick={() => setEditing(false)}><PackagePlus size={17}/>Novo produto</button>}<button className="primary" onClick={() => goTo('PDV')}><ShoppingCart size={18}/>Abrir PDV</button></div></div>
      {!suppliers.length && canManage && <div className="security-note"><AlertTriangle size={19}/><p>Cadastre primeiro o fornecedor. Depois ele aparecerá na busca do produto e ficará ligado às compras e reposições.</p></div>}
      {!!products.length && <label className="search catalog-search"><Search size={18}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar produto, SKU, tamanho ou fornecedor" /></label>}
      {!shown.length ? <EmptyState title={query ? 'Nenhum produto encontrado' : 'Seu catálogo está vazio'} text={query ? 'Tente outro nome, SKU, tamanho ou fornecedor.' : canManage ? 'Cadastre o fornecedor e depois o primeiro produto.' : 'Ainda não há produtos disponíveis para venda.'} action={!query && canManage && suppliers.length ? 'Cadastrar produto' : undefined} onAction={() => setEditing(false)} /> : <div className="catalog">{shown.map(product => <div className={product.active ? 'catalog-row' : 'catalog-row inactive'} key={product.id}><span className="product-swatch big" style={{background:product.color}}/><div><strong>{product.name}{product.size ? ` · ${product.size}` : ''}</strong><span>{product.sku} · {product.category} · {product.supplierName || 'Sem fornecedor'}</span></div><span>{canManage ? `${money(product.cost)} custo` : 'Custo protegido'}</span><b>{money(product.price)}</b><span className={!product.active ? 'stock inactive' : product.stock <= product.min ? 'stock low' : 'stock'}>{product.active ? `${product.stock} un.` : 'Inativo'}</span>{canManage ? <button className="icon-btn" onClick={() => setEditing(product)} title="Editar produto"><Pencil/></button> : product.active && <button className="icon-btn" onClick={() => add(product)} title="Adicionar ao carrinho"><Plus/></button>}</div>)}</div>}
    </section>
  </>
}

function ProductForm({ product, products, suppliers, onCancel, onSave }) {
  const [values, setValues] = useState(product ? { supplierId: product.supplierId || '', sku: product.sku, name: product.name, category: product.category, size: product.size || '', cost: product.cost?.toFixed(2) || '0.00', price: product.price.toFixed(2), stock: String(product.stock), min: String(product.min), color: product.color, active: product.active } : { supplierId: '', sku: '', name: '', category: '', size: '', cost: '', price: '', stock: '0', min: '0', color: '#70745a', active: true })
  ...
  return <div className="modal-backdrop"><form className="modal" onSubmit={async (event) => { event.preventDefault(); setSaving(true); await onSave(values); setSaving(false) }}>
    <div className="panel-head"><div><span className="section-label">CATÁLOGO</span><h3>{product ? 'Editar produto / variação' : 'Novo produto / variação'}</h3></div><button type="button" className="icon-btn" onClick={onCancel}><X/></button></div>
    <div className="form-grid"><SearchableSelect label="Fornecedor" value={values.supplierId} onChange={(supplierId) => setValues((current) => ({ ...current, supplierId }))} options={suppliers.map((supplier) => ({ id: supplier.id, label: supplier.name, keywords: `${supplier.contact_name} ${supplier.phone}` }))} placeholder="Digite para buscar o fornecedor" required/><label>Código / SKU<input required maxLength="60" value={values.sku} onChange={update('sku')} /></label><label>Nome do produto<input required maxLength="160" value={values.name} onChange={update('name')} /></label><label>Categoria<input required maxLength="80" list="product-categories" value={values.category} onChange={update('category')} /><datalist id="product-categories">{[...new Set(products.map((item) => item.category))].map((category) => <option key={category} value={category}/>)}</datalist></label><label>Tamanho / variação<input maxLength="40" list="product-sizes" value={values.size} onChange={update('size')} placeholder="Ex.: P, M, G, 38"/><datalist id="product-sizes">{['PP','P','M','G','GG','XG','34','36','38','40','42','44','46'].map((size) => <option key={size} value={size}/>)}</datalist></label><label>Cor de referência<input type="color" value={values.color} onChange={update('color')} /></label><label>Custo (R$)<input required min="0" step="0.01" type="number" value={values.cost} onChange={update('cost')} /></label><label>Preço de venda (R$)<input required min="0" step="0.01" type="number" value={values.price} onChange={update('price')} /></label>{!product && <label>Estoque inicial<input required min="0" step="1" type="number" value={values.stock} onChange={update('stock')} /></label>}<label>Estoque mínimo<input required min="0" step="1" type="number" value={values.min} onChange={update('min')} /></label>{product && <label>Situação<select value={values.active ? 'active' : 'inactive'} onChange={(event) => setValues((current) => ({ ...current, active: event.target.value === 'active' }))}><option value="active">Ativo</option><option value="inactive">Inativo</option></select></label>}</div>
    <p className="modal-help">Cada combinação de tamanho/cor possui seu próprio SKU e estoque. O fornecedor precisa existir antes e será escolhido pela busca.</p>
    <div className="modal-actions"><button type="button" className="secondary" onClick={onCancel}>Cancelar</button><button className="primary" disabled={saving || !values.supplierId}>{saving ? 'Salvando...' : product ? 'Salvar alterações' : 'Cadastrar produto'}</button></div>
  </form></div>
}

function Inventory({ products, movements, adjustStock, canManage }) {
  const [editing, setEditing] = useState(null)
  const [query, setQuery] = useState('')
  const shown = products.filter((product) => [product.name, product.sku, product.size, product.supplierName].some((value) => value?.toLowerCase().includes(query.toLowerCase())))
  const productById = Object.fromEntries(products.map((product) => [product.id, product]))
  return <><div className="page-intro"><div><span className="section-label">CONTROLE DE ESTOQUE</span><h2>Reposição sem surpresa</h2><p>Cada entrada ou correção gera uma movimentação auditável.</p></div><label className="search large"><Search size={18}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar produto, SKU, tamanho ou fornecedor" /></label></div>
    {!shown.length ? <EmptyState title={query ? 'Nenhum produto encontrado' : 'Nenhum produto em estoque'} text={query ? 'Tente outro nome, SKU, tamanho ou fornecedor.' : 'Cadastre um produto para começar.'} /> : <section className="inventory-grid">{shown.map(p => { const percent = Math.min(100, p.stock / (Math.max(1,p.min)*3) * 100); const low = p.stock <= p.min; return <article className="inventory-card" key={p.id}><div><span className="product-swatch big" style={{background:p.color}}/><span className={low?'status danger':'status'}>{low?'Repor':'Saudável'}</span></div><strong>{p.name}{p.size ? ` · ${p.size}` : ''}</strong><small>{p.sku} · {p.supplierName || 'Sem fornecedor'} · mínimo {p.min}</small><div className="stock-number"><b>{p.stock}</b><span>unidades</span></div><div className="progress"><i className={low?'low':''} style={{width:`${percent}%`}}/></div>{canManage && <><button className="secondary stock-action" onClick={() => setEditing(editing === p.id ? null : p.id)}>Registrar movimentação</button>{editing === p.id && <StockForm onSave={async (delta, reason) => { if (await adjustStock(p.id, delta, reason)) setEditing(null) }} />}</>}</article> })}</section>}
    <section className="panel inventory-history"><div className="panel-head"><div><span className="section-label">AUDITORIA</span><h3>Últimas movimentações</h3></div><History size={19}/></div>{!movements.length ? <EmptyState title="Nenhuma movimentação" text="Entradas, vendas e ajustes aparecerão aqui."/> : <div>{movements.filter((movement) => !query || shown.some((product) => product.id === movement.productId)).slice(0, 30).map((movement) => <article key={movement.id}><div><strong>{productById[movement.productId]?.name || 'Produto preservado'}</strong><small>{productById[movement.productId]?.sku || movement.productId.slice(0, 6)} · {movement.dateTime}</small></div><span>{inventoryMovementLabel(movement.type)}</span><b className={movement.quantity < 0 ? 'negative' : ''}>{movement.quantity > 0 ? '+' : ''}{movement.quantity}</b><em>{movement.reason}</em></article>)}</div>}</section>
  </>
}
...
function Purchases({ products, suppliers, purchases, createSupplier, receivePurchase }) {
  ...
  return <>
    ...
    {purchaseOpen && <PurchaseForm products={products.filter((product) => product.active)} suppliers={suppliers} onCancel={() => setPurchaseOpen(false)} onSave={async (values) => { if (await receivePurchase(values)) setPurchaseOpen(false) }} />}
    <div className="page-intro"><div><span className="section-label">ABASTECIMENTO</span><h2>Compras e fornecedores</h2><p>Receba mercadorias com custo, histórico e entrada automática no estoque.</p></div><div className="panel-actions"><button className="secondary" onClick={() => setSupplierOpen(true)}><Truck size={17}/>Novo fornecedor</button><button className="primary" disabled={!products.some((product) => product.active) || !suppliers.length} onClick={() => setPurchaseOpen(true)}><PackagePlus size={17}/>Registrar compra</button></div></div>
    {(!products.some((product) => product.active) || !suppliers.length) && <div className="security-note"><AlertTriangle size={19}/><p>Para registrar uma compra, cadastre pelo menos um produto ativo e um fornecedor. O botão será liberado automaticamente.</p></div>}
...
function PurchaseForm({ products, suppliers, onCancel, onSave }) {
  ...
  const [items, setItems] = useState([{ productId: '', quantity: '1', cost: '0.00' }])
  const [saving, setSaving] = useState(false)
...
function Cashier({ workspace, activeCash, sessions, movements, reconciliations, sales, openCash, moveCash, closeCash, reconcileCash }) {
  ...
  const sessionSales = selectedSession ? sales.filter((sale) => sale.cashSessionId === selectedSession.id) : []
  const sessionPaymentTotals = paymentMethodTotals(sessionSales)
  const sessionSalesTotal = sessionSales.reduce((total, sale) => total + sale.total, 0)
...
function Finance({ entries, customers, suppliers, createEntry, settleEntry }) {
  const [open,setOpen]=useState(false)
...
function SearchableSelect({ label, value, onChange, options, placeholder, required = false }) {
  ...
  const shown = options.filter((option) => `${option.label} ${option.keywords || ''}`.toLowerCase().includes(normalized)).slice(0, 12)
  return <div className="searchable-field"><span>{label}</span><div><Search size={16}/><input required={required} value={query} placeholder={placeholder} onFocus={() => setOpen(true)} onBlur={() => { window.setTimeout(() => setOpen(false), 120); if (!value) setQuery('') }} onChange={(event) => { setQuery(event.target.value); onChange(''); setOpen(true) }} />{open && <div className="searchable-options">{shown.length ? shown.map((option) => <button type="button" key={option.id || 'empty'} onMouseDown={(event) => event.preventDefault()} onClick={() => { onChange(option.id); setQuery(option.label); setOpen(false) }}><strong>{option.label}</strong>{option.keywords && <small>{option.keywords}</small>}</button>) : <p>Nenhum cadastro encontrado.</p>}</div>}</div></div>
}

function mapProduct(row) {
  return { id: row.id, supplierId: row.supplier_id, supplierName: row.supplier_name, sku: row.sku, name: row.name, category: row.category, size: row.size, cost: row.cost_cents == null ? null : row.cost_cents / 100, price: row.price_cents / 100, stock: row.stock_quantity, min: row.min_stock, color: row.color, active: row.active }
}

function mapInventoryMovement(row) {
  return { id: row.id, productId: row.product_id, type: row.movement_type, quantity: row.quantity_delta, reason: row.reason, dateTime: new Date(row.created_at).toLocaleString('pt-BR') }
}
...
function mapSale(row, payments = []) {
  ...
  return { id: `#${row.id.slice(0, 6).toUpperCase()}`, cashSessionId: row.cash_session_id, time: soldAt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }), customer: row.customer_name, payment, payments: payments.map((item) => ({ method: item.payment_method, amount: item.amount_cents / 100 })), total: row.total_cents / 100, status: 'Concluída', soldAt }
}
...
function mapFinancialEntry(row, payments = []) {
  ...
  return { id: row.id, type: row.entry_type, category: row.category, description: row.description, counterparty: row.counterparty_name, amount: row.amount_cents / 100, paymentMethod, dueDate: row.due_date, status: row.status }
}

function cashMovementLabel(type) { return ({ sale: 'Venda em dinheiro', supply: 'Reforço', withdrawal: 'Sangria' })[type] || 'Movimentação' }
function inventoryMovementLabel(type) { return ({ opening: 'Estoque inicial', purchase: 'Compra', adjustment: 'Ajuste', sale: 'Venda', return: 'Devolução' })[type] || 'Movimentação' }
